    ...
    <div className="space-y-10 max-w-5xl mx-auto">
      ...
      <div className="bg-blue-50 dark:bg-slate-800 border border-blue-200 dark:border-slate-700 rounded-xl p-5">
        ...
        <div className="flex flex-wrap gap-3 mt-3 text-xs text-slate-500">
          ...
          <span className="flex items-center gap-1 font-semibold text-blue-600"><Smartphone className="w-3.5 h-3.5" /> iPhone (App Store): 1242×2688 · 2688×1242 · 1284×2778 · or 2778×1284 px</span>
          <span className="flex items-center gap-1"><Smartphone className="w-3.5 h-3.5" /> Android Phone Screenshots: 9:16 · PNG/JPEG · 320–3840 px · max 8 MB</span>
      ...
      <IPhoneScreenshotSection />

      {/* Android Phone Screenshots */}
      <div>
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-1 flex items-center gap-2">
          <Smartphone className="w-5 h-5" style={{ color: '#61AF39' }} /> Android Phone Screenshots
          <Badge className="ml-2 text-xs" style={{ backgroundColor: '#61AF39' }}>2–8 Required · Google Play</Badge>
        </h3>
...
// ---------- iPhone App Store Screenshot Upload Section ----------

const IPHONE_VALID_SIZES = [
  { w: 1242, h: 2688, label: '1242 × 2688 px (iPhone XS Max / 6.5")' },
  { w: 2688, h: 1242, label: '2688 × 1242 px (Landscape 6.5")' },
  { w: 1284, h: 2778, label: '1284 × 2778 px (iPhone 12 Pro Max / 6.7")' },
  { w: 2778, h: 1284, label: '2778 × 1284 px (Landscape 6.7")' },
];

function validateIPhoneImage(file) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const match = IPHONE_VALID_SIZES.find(s => s.w === w && s.h === h);
      if (!match) {
        resolve({ ok: false, error: `Invalid size ${w}×${h}. Must be 1242×2688, 2688×1242, 1284×2778, or 2778×1284 px.` });
        return;
      }
      resolve({ ok: true, width: w, height: h, sizeLabel: match.label });
    };
...
function IPhoneUploadSlot({ index, entry, onAdd, onRemove }) {
  const inputRef = useRef(null);
  const [validating, setValidating] = useState(false);

  const isPortrait = !entry || (entry?.width && entry.height > entry.width);
  const aspectRatio = isPortrait ? '9/19.5' : '19.5/9';

  const handleFile = async (file) => {
    if (!file) return;
    setValidating(true);
    const result = await validateIPhoneImage(file);
    setValidating(false);
    if (!result.ok) {
    ...
    onAdd(index, { file, previewUrl, width: result.width, height: result.height, sizeLabel: result.sizeLabel, error: null });
  };
    ...
    <Card className="overflow-hidden">
      <div
        className={`relative flex items-center justify-center overflow-hidden cursor-pointer transition-colors ${
          hasError ? 'bg-red-50' : isEmpty ? 'bg-slate-50 hover:bg-slate-100' : 'bg-slate-100'
        }`}
        style={{ aspectRatio: entry?.previewUrl ? `${entry.width}/${entry.height}` : '9/19.5', minHeight: 140 }}
      ...
      >
        {entry?.previewUrl ? (
          <>
            <img src={entry.previewUrl} alt={`iPhone ${index + 1}`} className="w-full h-full object-contain" />
            <button
        ...
        <div className="flex gap-2">
          ...
          {entry?.previewUrl && (
            <Button size="sm" variant="outline" className="gap-1 text-xs" onClick={() => {
              ...
              a.download = `taper-payer-iphone-${index + 1}.png`;
              a.click();
            }}>
...
function IPhoneScreenshotSection() {
  const [slots, setSlots] = useState(Array(8).fill(null));

  const handleAdd = (index, entry) => setSlots(prev => { const next = [...prev]; next[index] = entry; return next; });
  const handleRemove = (index) => setSlots(prev => {
    const next = [...prev];
  ...
  return (
    <div>
      <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-1 flex items-center gap-2">
        <Smartphone className="w-5 h-5" style={{ color: '#2479C2' }} /> iPhone Screenshots
        <Badge className="ml-2 text-xs" style={{ backgroundColor: '#2479C2' }}>2–10 Required · App Store</Badge>
      </h3>
      <p className="text-slate-500 text-sm mb-2">PNG/JPEG · max 8 MB each · up to 10 screenshots</p>

      <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-xs text-blue-800 mb-2 flex items-start gap-2">
        <AlertCircle className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
        <div>
          <strong>Apple-required dimensions (portrait or landscape):</strong>
          <ul className="mt-1 space-y-0.5 list-disc list-inside">
            {IPHONE_VALID_SIZES.map(s => <li key={s.w + s.h}>{s.label}</li>)}
          </ul>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-xs text-amber-700 mb-5 flex items-start gap-2">
        <AlertCircle className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
        <span>Screenshots are validated automatically — wrong dimensions or file size will be flagged before upload.</span>
      </div>

      <p className="text-sm text-slate-600 mb-4">{uploadedCount}/8 screenshots uploaded</p>

      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
        {slots.map((entry, i) => (
          <IPhoneUploadSlot key={i} index={i} entry={entry} onAdd={handleAdd} onRemove={handleRemove} />
        ))}
      </div>
    </div>
  );
