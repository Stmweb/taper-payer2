...
Deno.serve(async (req) => {
  try {
    ...
    // Get the user's wallet address (either from AppUser entity or passed in)
    let { walletAddress } = await req.json().catch(() => ({}));
    if (!walletAddress) {
    ...
    const url = `https://buy.moonpay.com/?apiKey=${encodeURIComponent(publishableKey)}&defaultCurrencyCode=BNB&walletAddress=${encodeURIComponent(walletAddress)}`;
