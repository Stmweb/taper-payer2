...
async function sendMailgunEmail({ to, subject, html, text, from, replyTo }) {
...
Deno.serve(async (req) => {
  try {
    ...
    console.log('Raw body received:', JSON.stringify(rawText));
    ...
    if (type === 'contact') {
      ...
      await sendMailgunEmail({
        ...
        html: `
          <h2>Hi ${name},</h2>
          <p>Thank you for reaching out! We've received your message and our support team will get back to you within 24 hours.</p>
    ...
    // Transactional email (transfer/top-up confirmation)
    if (type === 'transactional') {
      const { to, subject, html, text } = data;
      const result = await sendMailgunEmail({ to, subject, html, text });
